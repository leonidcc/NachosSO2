...
Scheduler::Scheduler()
{
    queues = new List<List<Thread *> *>;
    for (int i = 0; i < NUM_COLAS; i++) {
      List<Thread *> *newQueue = new List<Thread *>;
      queues->Prepend(newQueue);
    }
}
